...
function TicTacToe() {
  ...
  const toggle = (e, num) => {
    ...
    if (count % 2 === 0) {
      e.target.innerHTML = ` <img src="${cross_image}" />`;
      data[num] = "x";
      setCount(count + 1);
    } else {
      e.target.innerHTML = ` <img src="${circle_image}" />`;
      data[num] = "o";
      setCount(count + 1);
    }
    checkWin();
  ...
  const won = (winner) => {
    setLock(true);
    // console.log(winner);     // why this winner is directly comparing?
    if (winner === "x") {
      titleRef.current.innerHTML = `congratulations: <img src="${cross_image}"/> Win`;
    }
    if (winner === "o") {
      titleRef.current.innerHTML = `congratulations: <img src="${circle_image}"/> Win`;
    }
  };
